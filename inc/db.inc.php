...
class SQLite {
    ...
    public function __construct($pathToSqliteDb)
    {
        ...
        if(!is_object($this->dbConnection)) {
            ...
            d::dbgError("SQLite connection object not returned for specified db", $pathToSqliteDb);
        }
    }
    ...
    public function execute($sql) {
        d::dbg($sql);
        if(empty($sql)) {
            d::dbgError("No query submitted");
            return false;
        } else {
            $ret = $this->dbConnection->exec($sql);
            if(!$ret) {
                d::dbgError($this->dbConnection->lastErrorMsg(), $sql);
                return false;
            } else {
...
class recordset {    
    ...
    public function __construct($sqliteObject=null) {
        ...
        if(!is_object($sqliteObject)) {
            d::dbgError("No result object passed", $sqliteObject);
            return false;
        } else {
